"""Explicit Mapping Workflow Run execution route."""
...
class ExecuteMappingRunRequest(BaseModel):
...
class MappingWorkflowService(Protocol):
    async def start(
        self,
...
def create_mapping_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: MappingWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix="/api/v1/tenants/{tenant_id}/models/{model_id}/mapping/runs",
        tags=["mapping"],
    )
    ...
    async def execute_run(
        ...
        command: ExecuteMappingRunRequest,
    ) -> AgentWorkflowRunStart:
