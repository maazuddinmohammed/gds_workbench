"""Explicit Dimensional Workflow Run execution route."""
...
class ExecuteDimensionalRunRequest(BaseModel):
...
class DimensionalWorkflowService(Protocol):
    async def start(
        self,
...
def create_dimensional_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: DimensionalWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix="/api/v1/tenants/{tenant_id}/models/{model_id}/dimensional/runs",
        tags=["dimensional"],
    )
    ...
    async def execute_run(
        ...
        command: ExecuteDimensionalRunRequest,
    ) -> AgentWorkflowRunStart:
