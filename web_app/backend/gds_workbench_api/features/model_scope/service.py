"""Active Model Scope read authorization and persistence."""
...
SELECT model_scope.model_scope_id,
       ...
       model_scope.created_time AS created_at,
       model_scope.updated_time AS updated_at
  FROM model.model AS target_model
  ...
  JOIN model.model_scope AS model_scope
   ...
   AND model_scope.object_id = eligible_object.object_id
...
_MODEL_SCOPE_CANDIDATES_SQL: LiteralString = f"""
{VISIBLE_OBJECTS_CTE}
...
class ModelScopeService(Protocol):
...
class ModelScopeReadDatabase(Protocol):
...
class DatabaseModelScopeService:
    def __init__(
        self,
        *,
        database: ModelScopeReadDatabase,
    ...
    async def list_candidates(
        ...
        *,
        ...
        collection = f"web_model_scope_candidates:{filter_digest}:{page_size}"
        ...
        return ModelScopeCandidatePage(
            ...
            items=tuple(ModelScopeCandidate.model_validate(row) for row in rows[:page_size]),
    ...
    async def list_scope(
        ...
        *,
        ...
        collection = f"web_model_scope:{filter_digest}:{page_size}"
        offset = self._cursors.decode(cursor, collection=collection)
        ...
        return ModelScopePage(
            model_id=model_id,
            model_revision=header["model_revision"],
            items=tuple(ModelScopeObject.model_validate(row) for row in rows[:page_size]),
            next_cursor=next_cursor,
        )
    ...
    async def read_scope_object(
        ...
        *,
        ...
        async with self._database.read_transaction(
            ...
            if row is None:
                raise ModelScopeObjectNotFoundError()
            attributes = await transaction.fetch_all(
                _MODEL_SCOPE_ATTRIBUTES_SQL,
                (object_id,),
            )
        return ModelScopeDetail.model_validate({**row, "attributes": attributes})
