...
class AnalysisInferenceWorkflowService(Protocol):
    async def start(
        self,
...
def create_analysis_inference_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: AnalysisInferenceWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix=("/api/v1/tenants/{tenant_id}/models/{model_id}/analysis/inference-runs"),
        tags=["analysis"],
    )
    ...
    async def execute_run(
        ...
        command: ExecuteAnalysisInferenceRunRequest,
    ) -> AgentWorkflowRunStart:
