"""Explicit Conceptual Workflow Run execution route."""
...
class ExecuteConceptualRunRequest(BaseModel):
...
class ConceptualWorkflowService(Protocol):
    async def start(
        self,
...
def create_conceptual_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: ConceptualWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix="/api/v1/tenants/{tenant_id}/models/{model_id}/conceptual/runs",
        tags=["conceptual"],
    )
    ...
    async def execute_run(
        ...
        command: ExecuteConceptualRunRequest,
    ) -> AgentWorkflowRunStart:
