"""Explicit Logical Workflow Run execution route."""
...
class LogicalWorkflowService(Protocol):
    async def start(
        self,
...
def create_logical_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: LogicalWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix="/api/v1/tenants/{tenant_id}/models/{model_id}/logical/runs",
        tags=["logical"],
    )
    ...
    async def execute_run(
        ...
        command: ExecuteLogicalRunRequest,
    ) -> AgentWorkflowRunStart:
