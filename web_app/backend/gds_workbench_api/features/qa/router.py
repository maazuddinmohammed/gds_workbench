"""Explicit QA Workflow Run execution route."""
...
class ExecuteQARunRequest(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, strict=True)

    expected_model_revision: int = Field(gt=0)


class QAWorkflowService(Protocol):
    async def start(
        self,
...
def create_qa_workflow_router(
    *,
    identity_provider: IdentityProvider,
    service: QAWorkflowService,
) -> APIRouter:
    router = APIRouter(
        prefix="/api/v1/tenants/{tenant_id}/models/{model_id}/qa/runs",
        tags=["qa"],
    )
...
__all__ = ["ExecuteQARunRequest", "QAWorkflowService", "create_qa_workflow_router"]
