"""Validate and reconcile complete per-System QA candidates."""
...
from .context import QAExecutionContext, QASystemAuthoringContext
...
class _AgentValidationGroup(BaseModel):
    ...
    validation_group_description: str | None = Field(
        default=None,
        min_length=1,
        description="Optional purpose and QA coverage; use null when unnecessary.",
    )
...
class _AgentQASystemCandidate(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True, strict=True)
...
class ValidatedQASystemCandidate:
    system_ref: str
    groups: tuple[ValidationGroupRecord, ...]
    checks: tuple[ValidationCheckRecord, ...]


class QASystemCandidateValidator:
    def __init__(self, *, context: QASystemAuthoringContext) -> None:
        self._context = context

    def output_schema(self) -> dict[str, JsonValue]:
        return cast(dict[str, JsonValue], _AgentQASystemCandidate.model_json_schema())
            ...
            return AgentCandidateValidation(
                issues=(
                    AgentValidationIssue(
                        ...
                        path=(),
                        message="The candidate does not match the required QA schema.",
                    ),
                )
            )
        return AgentCandidateValidation(issues=())

    def parse_validated(self, candidate: JsonValue) -> ValidatedQASystemCandidate:
        try:
            encoded = json.dumps(
                candidate,
                ensure_ascii=False,
            ...
            raise InvalidRequestError("The QA candidate is invalid.") from None
        if len(encoded) > _MAX_CANDIDATE_BYTES:
            raise _CandidateIssueError(
                AgentValidationIssue(
                    code="candidate.too_large",
                    path=(),
                    message="The QA candidate exceeds its bounded size.",
                )
            )
        parsed = _AgentQASystemCandidate.model_validate(candidate, strict=True)
        ...
        if check_count > _MAX_CHECKS_PER_SYSTEM:
            raise _CandidateIssueError(
                AgentValidationIssue(
                    ...
                    path=("validation_groups",),
                    message="The QA candidate contains too many Validation Checks.",
                )
            )
            ...
            try:
                groups.append(
                    ValidationGroupRecord(
                        ...
                        mapping_context_digest=self._context.mapping_context_digest,
                        code_context_digest=self._context.code_context_digest,
                        is_active=True,
                    )
                )
        ...
        return ValidatedQASystemCandidate(
...
def reconcile_qa_candidates(
    *,
    context: QAExecutionContext,
    candidates: tuple[ValidatedQASystemCandidate, ...],
    ...
    if (
        ...
        raise InvalidRequestError("The QA candidate and frozen System coverage differ.")
    ...
    for system_ref, system in systems_by_ref.items():
        ...
        if (
            ...
            raise InvalidRequestError("The QA candidate context is ambiguous.")

        for key, desired in desired_groups.items():
            if desired != applied_groups.get(key):
