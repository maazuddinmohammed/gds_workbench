"""Execute one already-running QA authoring Workflow Run."""
...
from gds_workbench_api.capabilities import QA_AGENT_EXECUTION_MODE
...
from .context import PostgresQAContextRepository, QAExecutionContext

_logger = logging.getLogger(__name__)


class QAExecutionDatabase(Protocol):
...
class QAPlanRepository(Protocol):
    async def load(
        self,
...
class QAContextRepository(Protocol):
    async def load(
        self,
        transaction: ReadTransaction,
        *,
...
class QAChangeSetHandoff(Protocol):
    async def finalize(
        self,
...
class QANoOpCompleter(Protocol):
    async def complete(
        self,
        principal: RequestPrincipal,
        *,
...
class QAExecutionFailedError(WorkbenchError):
    def __init__(self) -> None:
        super().__init__(
            code="qa_execution_failed",
            message="QA authoring failed before a validated draft was committed.",
        )


class QAFinalizationFailedError(WorkbenchError):
    def __init__(self) -> None:
        super().__init__(
            code="qa_finalization_failed",
            message="QA finalization outcome could not be confirmed.",
        )


type QAExecutionResult = WorkflowChangeSetHandoffResult | AuthoringNoOpReceipt


class QARunLifecycle(Protocol):
    async def start(
        self,
        principal: RequestPrincipal,
        *,
...
class QAWorkflow:
    def __init__(self, *, lifecycle: QARunLifecycle, executor: QAExecutor) -> None:
...
class DatabaseQAExecutor:
    def __init__(
        self,
        *,
        database: QAExecutionDatabase,
        authorizer: AuthorizationService,
        agent_executor: AgentExecutor,
        handoff: QAChangeSetHandoff,
        no_op: QANoOpCompleter,
        lifecycle: QALifecycle,
        plan_repository: QAPlanRepository | None = None,
        context_repository: QAContextRepository | None = None,
        context_policy: AgentContextPolicy | None = None,
    ) -> None:
        self._database = database
        self._authorizer = authorizer
        self._plan_repository = plan_repository or PostgresAgentRunPlanRepository()
        self._context_repository = context_repository or PostgresQAContextRepository()
        ...
        *,
        ...
        try:
            ...
            await progress.append(
                attempt=1,
                stage="qa.validation_generation",
                status="running",
                message=f"QA generation started for {system_count} Systems.",
                current=0,
                total=system_count,
                finding_count=0,
            )
            ...
            stage_plan = plan.model_copy(
                update={"workflow_execution_mode": QA_AGENT_EXECUTION_MODE}
            )
            candidates: list[ValidatedQASystemCandidate] = []
            ...
            for position, system in enumerate(context.systems, start=1):
                validator = QASystemCandidateValidator(context=system)
                ...
                if position in progress_points:
                    message = f"QA generation validated {position} of {system_count} Systems."
                    if warning_seen:
                ...
                return await self._no_op.complete(
                    ...
                    request=AuthoringNoOpRequest(
                        ...
                        final_event=progress.event(
                            attempt=highest_attempt,
                            stage="qa.backend_validation",
                            status="warning" if warning_seen else "running",
                            message="QA authoring completed with no effective change.",
                            current=1,
                            total=1,
                            finding_count=0,
                        ),
            ...
            finalized = await self._handoff.finalize(
                ...
                final_event=progress.event(
                    ...
                    stage="qa.backend_validation",
                    status="warning" if warning_seen else "running",
                    message="QA candidate is ready in a validated draft.",
                    current=1,
                    total=1,
            ...
            if finalization_attempted:
                _logger.warning(
                    "QA Workflow Run finalization remains pending.",
                    extra={
                        "workflow_run_id": workflow_run_id,
                        "model_id": model_id,
            ...
            try:
                ...
                _logger.warning(
                    "QA failure state could not be persisted.",
                    extra={
                        "workflow_run_id": workflow_run_id,
                        "model_id": model_id,
                        "failure_code": safe_error.code[:100],
    ...
    def _validate_plan(
        ...
        *,
        ...
        if (
            ...
            raise InvalidRequestError("The QA run does not use the fixed execution path.")
...
def _safe_execution_error(
    ...
    *,
    ...
    if isinstance(error, WorkbenchError):
        return error
    if finalization_attempted:
        return QAFinalizationFailedError()
    return QAExecutionFailedError()
