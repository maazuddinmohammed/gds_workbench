...
export function ValidationRunDialog({
  ...
  const modal = (
    <div className="dialog-scrim validation-dialog-scrim" role="presentation">
      <section
        ref={dialog}
        className="run-configuration-dialog validation-run-dialog"
        role="dialog"
        aria-modal="true"
        aria-labelledby="validation-run-heading"
        onKeyDown={(event) => {
          if (event.key === "Escape") {
      ...
      >
        <header className="drawer-header">
          ...
          <button
            ...
            type="button"
            aria-label="Close Configure Validation run"
        ...
        >
          <section className="validation-run-systems" aria-labelledby="validation-system-selection-heading">
            <header>
              <div>
                <strong id="validation-system-selection-heading">Systems</strong>
            ...
            <fieldset disabled={runMutation.isPending}>
              <legend className="sr-only">Eligible Validation Systems</legend>
              <ul className="validation-system-selection-list">
                {systems.map((system) => (
                  <li key={system.system_id}>
                    <label>
                      ...
                      <span className={`status-badge ${system.has_applied_validation ? "is-success" : "is-neutral"}`}>
                        {system.has_applied_validation ? "Applied Validation" : "No applied Validation"}
                      </span>
                    </label>
                  </li>
                ))}
            ...
            <p className="validation-selection-summary" role="status">
          ...
          {runMutation.isError ? (
            <p className="inline-error" role="alert">
              {validationRunError(runMutation.error, pendingWorkflowRunId !== null)}
            </p>
          ) : null}
            ...
            <div>
              ...
              >
                {runMutation.isPending
                  ...
                  : pendingWorkflowRunId === null ? "Create and start Validation" : "Retry start"}
              </button>
            </div>
          </footer>
        </form>
...
function validationRunError(error: Error, runWasCreated: boolean): string {
  ...
  if (runWasCreated) return "The Validation run remains queued because it could not be started.";
  if (error instanceof ApiError && error.status === 403) {
    return "You no longer have permission or the required Tenant Lock to run Validation.";
  }
  if (error instanceof ApiError && error.status === 409) {
    return "The Model or Validation context changed. Refresh before creating another run.";
  }
  return "The Validation run could not be created or started.";
}
