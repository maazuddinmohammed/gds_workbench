...
describe("Dimensional HTTP adapter", () => {
  it("owns exact normalized collection and detail read transports", async () => {
    ...
    expect(fetcher.mock.calls.map(([input]) => String(input))).toEqual([
      "/api/v1/tenants/7/models/18/dimensional/objects?status=needs_review&locked=false&name_exact=sales+fact&name_prefix=sales&page_size=25&cursor=opaque%2B%2F%3D",
      ...
      "/api/v1/tenants/7/models/18/dimensional/relationships?status=needs_review&locked=false&name_prefix=sales&dimensional_entity_id=71&page_size=75&cursor=relationships%2B%2F%3D",
