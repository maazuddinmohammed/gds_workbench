...
type ModelScopeScreenApi = Pick<TenantsApi, "readTenantHome">
  & Pick<ModelsApi, "readModel">
  & ModelScopeApi;

export function ModelScopeScreen({
  api,
  tenantId,
  modelId,
}: {
  api: ModelScopeScreenApi;
  tenantId: number;
  modelId: number;
}) {
  const validIds = validTenantModelIds(tenantId, modelId);
  const [filters, setFilters] = useState<ModelScopeFilters>({});
  ...
  const scopeQuery = useQuery({
    queryKey: ["model-scope", tenantId, modelId, filters],
    queryFn: () => api.listModelScope(tenantId, modelId, filters),
    enabled: validIds,
  });
  const detailQuery = useQuery({
    queryKey: ["model-scope-object", tenantId, modelId, detailObjectId],
    queryFn: () => api.readModelScopeObject(tenantId, modelId, detailObjectId as number),
    enabled: validIds && detailObjectId !== null,
  });

  if (!validIds) return <ErrorPage />;
  if (homeQuery.isPending || modelQuery.isPending) {
    return <LoadingPage label="Loading active Scope" />;
  }
  ...
  return (
    <TenantWorkspace home={homeQuery.data} activeNav="models" model={modelQuery.data}>
      <ModelWorkspaceShell model={modelQuery.data} activeStage="scope">
        <ScopeView
          modelScopeObjectCount={modelQuery.data.model_scope_object_count}
...
function ScopeView({
  ...
  modelScopeObjectCount: number;
  objects: ModelScopeObject[];
  isLoading: boolean;
  isError: boolean;
  selectedObjectId: number | null;
  detail: ModelScopeDetail | undefined;
  isDetailLoading: boolean;
  isDetailError: boolean;
  onShowDetails: (objectId: number) => void;
  onCloseDetails: () => void;
  onFiltersChange: (filters: ModelScopeFilters) => void;
}) {
  const columns = useMemo<ColumnDef<ModelScopeObject>[]>(() => [
    {
  ...
  return (
    <div className="scope-page page-enter">
      <header className="section-bar">
        ...
        <span>{objects.length} of {modelScopeObjectCount} Objects shown</span>
      </header>
      <ScopeFilterForm onApply={onFiltersChange} />
      <div className={`scope-data-layout${selectedObjectId ? " has-inspector" : ""}`}>
        <section className="scope-ledger" aria-label="Active Scope ledger">
          {isLoading ? (
            <div className="surface-state" aria-busy="true">Loading Scope Objects…</div>
          ) : isError ? (
          ...
          {!isLoading && !isError && !objects.length ? (
            <div className="empty-state compact">No active Scope Objects match these filters.</div>
          ) : null}
        </section>
        {selectedObjectId ? (
          <ScopeDetailDrawer
...
function ScopeDetailDrawer({
  ...
  detail: ModelScopeDetail | undefined;
  fallback: ModelScopeObject | undefined;
  isLoading: boolean;
  isError: boolean;
  onClose: () => void;
}) {
  ...
  const eligibility = detail ? [
    ["Bronze source", detail.is_bronze_source_eligible],
  ...
  return (
    <aside
      ...
      aria-label="Scope Object details"
      onKeyDown={(event) => {
    ...
    >
      <header>
        <div>
          ...
          <h2>{object?.object_name ?? "Scope Object"}</h2>
        </div>
...
function ScopeFilterForm({ onApply }: { onApply: (filters: ModelScopeFilters) => void }) {
  const form = useForm({
    defaultValues: {
      zone: "",
      systemCode: "",
  ...
  return (
    <form
      ...
      aria-label="Filter active Scope"
      onSubmit={(event) => {
