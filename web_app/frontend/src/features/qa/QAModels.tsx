...
export function QAModels({ api, tenantId }: { api: QAApi; tenantId: number }) {
  const query = useInfiniteQuery({
    queryKey: qaQueryKeys.models(tenantId),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          ...
          aria-label={`Open ${row.original.model_name} QA`}
          to="/tenants/$tenantId/qa/models/$modelId"
  ...
  return (
    <section className="models-page qa-models page-enter">
      <header className="models-commandbar">
        ...
        <div className="mapping-model-actions">
          <span className="models-context-note">Choose an active Model to review or author QA</span>
          <button
            className="button button-secondary button-small"
            type="button"
      ...
      {query.isPending ? (
        <div className="surface-state" aria-busy="true">Loading Models for QA…</div>
      ) : query.error instanceof ApiError && query.error.status === 403 ? (
        <div className="surface-state is-error" role="alert">
          You do not have permission to view Models for QA.
        </div>
      ) : query.isError ? (
        <div className="surface-state is-error" role="alert">
          Models for QA could not be loaded.
        </div>
      ) : models.length === 0 ? (
        <div className="empty-state compact">No active Models are available for QA.</div>
      ) : (
        <div className="models-table-scroll table-scroll">
          <table aria-label="Models for QA">
            <thead>
