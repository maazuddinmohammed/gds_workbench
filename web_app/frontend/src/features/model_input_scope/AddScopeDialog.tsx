  ...
  return <div className="dialog-scrim" role="presentation">
    <section ref={dialog} className="run-configuration-dialog scope-add-dialog" role="dialog" aria-modal="true" aria-labelledby="scope-add-title" tabIndex={-1}
      ...
      <form className="scope-add-filters" onSubmit={(event) => { event.preventDefault(); const sourceTenant = tenants.find((item) => item.tenant_id === sourceTenantId); if (sourceTenant && system && zone) setSearch({ sourceTenantCode: sourceTenant.tenant_code, systemCode: system, zone, objectName: name }); }}>
        <label>Source Tenant<select ref={firstInput} value={sourceTenantId || ""} disabled={busy} onChange={(event) => { setSourceTenantId(Number(event.target.value)); setSystem(""); setZone(""); resetSearch(); }}><option value="">Choose Source Tenant</option>{tenants.map((item) => <option key={item.tenant_id} value={item.tenant_id}>{item.tenant_code}</option>)}</select></label>
        <label>System<select value={system} disabled={!sourceTenantId || busy} onChange={(event) => { setSystem(event.target.value); setZone(""); resetSearch(); }}><option value="">Choose System</option>{systems.map((item) => <option key={item.system_code} value={item.system_code}>{item.system_code}</option>)}</select></label>
