-- GDS ETL Workbench Release 1: exact seven-table Logical and Dimensional layers.
...
CREATE TABLE workflow.logical_submodel (
    ...
    CONSTRAINT ck_logical_submodel_name CHECK (core.is_nonblank(logical_submodel_name)),
    CONSTRAINT ck_logical_submodel_definition CHECK (
        core.is_nonblank(logical_submodel_definition)
    ),
...
CREATE TABLE workflow.logical_entity (
    ...
    CONSTRAINT ck_logical_entity_name CHECK (core.is_nonblank(logical_entity_name)),
    CONSTRAINT ck_logical_entity_definition CHECK (
        core.is_nonblank(logical_entity_definition)
    ),
    ...
    CONSTRAINT ck_logical_entity_type_detail CHECK (
        (logical_entity_type = 'other' AND core.is_nonblank(logical_entity_type_detail))
        OR (logical_entity_type <> 'other' AND logical_entity_type_detail IS NULL)
    ),
    CONSTRAINT ck_logical_entity_grain CHECK (core.is_nonblank(logical_entity_grain)),
...
CREATE TABLE workflow.logical_attribute (
    ...
    CONSTRAINT ck_logical_attribute_name CHECK (
        core.is_nonblank(logical_attribute_name)
    ),
    CONSTRAINT ck_logical_attribute_definition CHECK (
        core.is_nonblank(logical_attribute_definition)
    ),
    CONSTRAINT ck_logical_attribute_data_type CHECK (
        core.is_nonblank(logical_attribute_data_type)
    ),
...
CREATE TABLE workflow.logical_entity_source_mapping (
    ...
    CONSTRAINT ck_logical_entity_source_rationale CHECK (
        core.is_nonblank(logical_entity_source_mapping_rationale)
    ),
...
CREATE TABLE workflow.logical_attribute_source_mapping (
    ...
    CONSTRAINT ck_logical_attribute_source_rationale CHECK (
        core.is_nonblank(logical_attribute_source_mapping_rationale)
    ),
...
CREATE TABLE workflow.logical_relationship (
    ...
    CONSTRAINT ck_logical_relationship_name CHECK (
        core.is_nonblank(logical_relationship_name)
    ),
    CONSTRAINT ck_logical_relationship_definition CHECK (
        core.is_nonblank(logical_relationship_definition)
    ),
    ...
    CONSTRAINT ck_logical_relationship_bases CHECK (
        core.is_nonblank(logical_relationship_basis)
        AND core.is_nonblank(logical_relationship_cardinality_basis)
    ),
...
-- Dimensional persists the same seven-family parent/child pattern.
CREATE TABLE workflow.dimensional_submodel (
    ...
    CONSTRAINT ck_dimensional_submodel_name CHECK (
        core.is_nonblank(dimensional_submodel_name)
    ),
    CONSTRAINT ck_dimensional_submodel_definition CHECK (
        core.is_nonblank(dimensional_submodel_definition)
    ),
...
CREATE TABLE workflow.dimensional_entity (
    ...
    CONSTRAINT ck_dimensional_entity_name CHECK (
        core.is_nonblank(dimensional_entity_name)
    ),
    CONSTRAINT ck_dimensional_entity_definition CHECK (
        core.is_nonblank(dimensional_entity_definition)
    ),
    ...
    CONSTRAINT ck_dimensional_entity_grain CHECK (
        (
            ...
            AND core.is_nonblank(dimensional_entity_grain_definition)
        ) OR (
            dimensional_entity_type = 'dimension'
            AND (
                dimensional_entity_grain_definition IS NULL
                OR core.is_nonblank(dimensional_entity_grain_definition)
            )
        )
    ),
...
CREATE TABLE workflow.dimensional_attribute (
    ...
    CONSTRAINT ck_dimensional_attribute_name CHECK (
        core.is_nonblank(dimensional_attribute_name)
    ),
    CONSTRAINT ck_dimensional_attribute_definition CHECK (
        core.is_nonblank(dimensional_attribute_definition)
    ),
    CONSTRAINT ck_dimensional_attribute_data_type CHECK (
        core.is_nonblank(dimensional_attribute_data_type)
    ),
    ...
    CONSTRAINT ck_dimensional_measure_policy CHECK (
        (
            ...
            AND core.is_nonblank(dimensional_attribute_default_aggregation)
            AND (
                dimensional_attribute_additivity = 'additive'
                OR core.is_nonblank(dimensional_attribute_aggregation_basis)
            )
        ) OR (
            dimensional_attribute_role <> 'measure'
...
CREATE TABLE workflow.dimensional_entity_source_mapping (
    ...
    CONSTRAINT ck_dimensional_entity_source_role CHECK (
        core.is_nonblank(dimensional_entity_source_role)
    ),
    ...
    CONSTRAINT ck_dimensional_entity_source_rationale CHECK (
        core.is_nonblank(dimensional_entity_source_mapping_rationale)
    ),
...
CREATE TABLE workflow.dimensional_attribute_source_mapping (
    ...
    CONSTRAINT ck_dimensional_attribute_source_rationale CHECK (
        core.is_nonblank(dimensional_attribute_source_mapping_rationale)
    ),
...
CREATE TABLE workflow.dimensional_relationship (
    ...
    CONSTRAINT ck_dimensional_relationship_name CHECK (
        core.is_nonblank(dimensional_relationship_name)
    ),
    CONSTRAINT ck_dimensional_relationship_definition CHECK (
        core.is_nonblank(dimensional_relationship_definition)
    ),
    CONSTRAINT ck_dimensional_relationship_kind CHECK (
        core.is_nonblank(dimensional_relationship_kind)
    ),
    ...
    CONSTRAINT ck_dimensional_relationship_role CHECK (
        ...
        OR core.is_nonblank(dimensional_relationship_role_name)
    ),
    ...
    CONSTRAINT ck_dimensional_relationship_bases CHECK (
        core.is_nonblank(dimensional_relationship_basis)
        AND core.is_nonblank(dimensional_relationship_cardinality_basis)
    ),
...
-- Every effective layer mutation participates in the one-revision transaction guard.
...
-- Supporting read, impact, parent, and lock indexes.
...
CREATE INDEX ix_logical_entity_submodel_entity ON workflow.logical_entity_submodel (model_id, logical_entity_id);
...
CREATE INDEX ix_logical_relationship_from ON workflow.logical_relationship (model_id, logical_relationship_from_entity_id, logical_relationship_from_attribute_id);
...
CREATE INDEX ix_logical_submodel_locked ON workflow.logical_submodel (model_id, logical_submodel_id) WHERE logical_submodel_is_locked;
CREATE INDEX ix_logical_entity_locked ON workflow.logical_entity (model_id, logical_entity_id) WHERE logical_entity_is_locked;
CREATE INDEX ix_logical_entity_submodel_locked ON workflow.logical_entity_submodel (model_id, logical_entity_submodel_id) WHERE logical_entity_submodel_is_locked;
CREATE INDEX ix_logical_attribute_locked ON workflow.logical_attribute (model_id, logical_attribute_id) WHERE logical_attribute_is_locked;
CREATE INDEX ix_logical_entity_source_locked ON workflow.logical_entity_source_mapping (model_id, logical_entity_source_mapping_id) WHERE logical_entity_source_mapping_is_locked;
CREATE INDEX ix_logical_attribute_source_locked ON workflow.logical_attribute_source_mapping (model_id, logical_attribute_source_mapping_id) WHERE logical_attribute_source_mapping_is_locked;
CREATE INDEX ix_logical_relationship_locked ON workflow.logical_relationship (model_id, logical_relationship_id) WHERE logical_relationship_is_locked;
CREATE INDEX ix_dimensional_submodel_locked ON workflow.dimensional_submodel (model_id, dimensional_submodel_id) WHERE dimensional_submodel_is_locked;
CREATE INDEX ix_dimensional_entity_locked ON workflow.dimensional_entity (model_id, dimensional_entity_id) WHERE dimensional_entity_is_locked;
CREATE INDEX ix_dimensional_entity_submodel_locked ON workflow.dimensional_entity_submodel (model_id, dimensional_entity_submodel_id) WHERE dimensional_entity_submodel_is_locked;
CREATE INDEX ix_dimensional_attribute_locked ON workflow.dimensional_attribute (model_id, dimensional_attribute_id) WHERE dimensional_attribute_is_locked;
CREATE INDEX ix_dimensional_entity_source_locked ON workflow.dimensional_entity_source_mapping (model_id, dimensional_entity_source_mapping_id) WHERE dimensional_entity_source_mapping_is_locked;
CREATE INDEX ix_dimensional_attribute_source_locked ON workflow.dimensional_attribute_source_mapping (model_id, dimensional_attribute_source_mapping_id) WHERE dimensional_attribute_source_mapping_is_locked;
CREATE INDEX ix_dimensional_relationship_locked ON workflow.dimensional_relationship (model_id, dimensional_relationship_id) WHERE dimensional_relationship_is_locked;
