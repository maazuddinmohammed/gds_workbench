-- GDS ETL Workbench Release 1: generated Code and QA Model Sections.
...
CREATE TABLE workflow.generated_code (
    ...
    generated_code_digest CHAR(64) NOT NULL,
    ...
    CONSTRAINT fk_generated_code_scope FOREIGN KEY (model_id, object_id)
        REFERENCES model.model_scope (model_id, object_id) ON DELETE NO ACTION,
    CONSTRAINT uq_generated_code_model_object UNIQUE (model_id, object_id),
    CONSTRAINT ck_generated_code_entity_type CHECK (
    ...
    CONSTRAINT ck_generated_code_digests CHECK (
        mapping_context_digest ~ '^[0-9a-f]{64}$'
        AND source_context_digest ~ '^[0-9a-f]{64}$'
        AND generated_code_digest ~ '^[0-9a-f]{64}$'
        AND generated_code_digest = encode(
            sha256(convert_to(generated_code_content, 'UTF8')),
            'hex'
        )
    ),
    CONSTRAINT ck_generated_code_status CHECK (
        generated_code_status IN (
            'active', 'needs_review', 'inactive', 'deprecated'
        )
    )
);

CREATE INDEX ix_generated_code_workflow_run
    ON workflow.generated_code (workflow_run_id, model_id)
    WHERE workflow_run_id IS NOT NULL;
CREATE INDEX ix_generated_code_active_model
    ON workflow.generated_code (model_id, object_id)
    WHERE generated_code_status = 'active';
...
CREATE INDEX ix_validation_group_pipeline_lookup
    ...
    WHERE is_active;
CREATE INDEX ix_validation_group_workflow_run
    ON workflow.validation_group (workflow_run_id, model_id)
    WHERE workflow_run_id IS NOT NULL;
