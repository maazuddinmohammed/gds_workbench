...
-- database/19_runtime_integrity.sql have completed.
...
BEGIN
    ...
    IF (
        ...
        SELECT count(*)
          FROM information_schema.columns AS column_record
         WHERE column_record.table_schema = 'application'
           AND column_record.table_name = 'workflow_run_system_selection'
           AND column_record.column_name IN (
                   'workflow_run_id',
                   'model_id',
                   'system_id',
    ...
    IF to_regclass('workflow.mapping_object') IS NULL
       ...
       OR to_regclass('workflow.validation_check') IS NULL
    ...
    IF (
        ...
        SELECT count(*)
          FROM pg_catalog.pg_constraint AS constraint_record
         WHERE constraint_record.conrelid =
               'workflow.validation_group'::REGCLASS
           AND constraint_record.conname IN (
                   'fk_validation_group_model_tenant',
                   'fk_validation_group_system',
                   'fk_validation_group_workflow_run',
                   'ck_validation_group_digests'
               )
           AND constraint_record.convalidated
    ) <> 4 OR (
        SELECT count(*)
          FROM pg_catalog.pg_constraint AS constraint_record
         WHERE constraint_record.conrelid =
               'workflow.validation_check'::REGCLASS
           AND constraint_record.conname IN (
                   'fk_validation_check_group',
                   'ck_validation_check_operator',
                   'ck_validation_check_value_type',
                   'ck_validation_check_assertion_shape',
                   'ck_validation_check_literal_type'
               )
           AND constraint_record.convalidated
    ) <> 5 OR to_regclass(
        'workflow.ux_validation_group_scope_name_ci'
    ) IS NULL OR to_regclass(
        'workflow.ux_validation_check_group_name_ci'
    ) IS NULL OR (
        SELECT count(*)
          FROM information_schema.columns AS column_record
         WHERE column_record.table_schema = 'mcp'
       ...
       OR EXISTS (
           SELECT 1
             FROM unnest(ARRAY[
                  ...
                  ]) AS new_model_section_table(name)
            WHERE NOT has_table_privilege(
                      'gds_app_write', new_model_section_table.name,
...
BEGIN
    IF to_regclass('application.metadata_review_event') IS NULL
       OR NOT EXISTS (
           ...
           SELECT 1 FROM unnest(ARRAY['gds_web_write', 'gds_app_write']) AS runtime(name)
