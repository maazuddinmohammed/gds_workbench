...
- only affected Model records and their direct source/dependency records;
...
5. If the stopping boundary is local draft, save the complete checked records only in
   `GDS/model-change-set` using the GDS Data Workbench, then stop without a lock or MCP
   mutation.
6. Otherwise, check the Tenant Lock, pause for approval, and acquire it. Re-read
   ...
   Stage approval.
7. Stage complete approved dataset replacements with the latest draft revision.
   Validate on the server; repair one failed phase at a time and revalidate.
8. If the stopping boundary is validated draft, hand off the authoritative action
   review, release the caller-owned Tenant Lock, and stop without Apply. Report any
   release failure as a blocker.
9. If the boundary is applied model, show the validated action review and pause for a
...
[Local draft: complete records are saved in `GDS/model-change-set`, locally checked,
and no Tenant Lock or server mutation occurred.]
...
Keep ordinary checkpoints to three bullets and 120 words. Expand only for a precise
blocker, validation warnings/truncation, Stage review, or authoritative Apply review.
```
