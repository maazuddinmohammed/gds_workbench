...
Use these only when one complete dataset cannot fit the normal MCP request. First run
the plugin's `prepare-stage-batch.js`; chunks contain typed records, never arbitrary
files or file paths.

### `begin_metadata_stage_batch`

Provide Tenant/Change Set IDs, exact revision, dataset, total record/chunk counts, and
the manifest `batch_sha256`. It creates or resumes the exact active manifest without
changing the draft revision.

### `put_metadata_stage_chunk`

Provide the Batch ID, same dataset, 1-based index, complete chunk records, and
`chunk_sha256`. Each chunk has at most 5,000 records and 450 KiB canonical JSON. Exact
retries are safe; conflicting reuse is rejected.

### `commit_metadata_stage_batch`

Provide the Batch ID and original expected revision. It verifies all chunks and
atomically performs the same complete-list replacement as normal Stage. It increments
the revision once and is replay-safe for that Batch.

Chunks are not Change Set datasets. Get, Validate, and Apply cannot see them before
Commit. A Stage Batch is for exactly one nonempty dataset; use normal Stage for an
empty replacement.
