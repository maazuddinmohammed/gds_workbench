...
There is no separate build or arbitrary file-upload MCP tool. Build JSON arrays
locally. Use `stage_metadata_change_set` when the complete approved input fits one
request. Use the typed Stage Batch tools only when one complete dataset is too large.
...
batch. The combined serialized input must remain within the transport request limit.
...
## Large dataset Stage Batch

After the same local validation, review, and Stage approval, run the shared
`scripts/prepare-stage-batch.js` helper with `--kind metadata`. It creates one
manifest plus 1–64 typed chunk files, each at most 450 KiB. Do not print chunk bodies.

1. Call `begin_metadata_stage_batch` with the manifest counts and `batch_sha256`.
2. Call `put_metadata_stage_chunk` for each ordered chunk using its exact records,
   index, and `chunk_sha256`. Exact retries are idempotent.
3. Call `commit_metadata_stage_batch` at the original revision.
4. Record the one returned revision and reviewed complete-dataset hash locally.

Begin and Put do not change the Change Set. Commit verifies the complete manifest,
replaces the dataset once, clears prior validation, and increments the revision once.
Never mix normal Stage and an active Stage Batch for the same dataset. On a revision
conflict, inspect and reconcile the Change Set; do not commit the stale Batch.
