...
7. For normal-size input, call `stage_metadata_change_set` once with one to 16 unique
   complete accumulated lists. When one dataset exceeds the request limit, use
   `prepare-stage-batch.js`, `begin_metadata_stage_batch`, ordered
   `put_metadata_stage_chunk` calls, then `commit_metadata_stage_batch`. Use the same
   approved complete list and original revision; record only Commit's returned revision.
...
- Stage Batch chunks are temporary transport only. Begin/Put never modify the Change
  Set; Commit performs one complete replacement. Never treat chunks as append records.
