...
def _context(
    *,
    ...
    return ValidationSystemAuthoringContext(
        system_ref="system_1",
        tenant_code="acme",
        system_code="erp",
        applied_groups=groups,
        applied_checks=checks,
        current_group_names=tuple(group.validation_group_name for group in groups),
...
def test_candidate_schema_publishes_the_scalar_runtime_contract() -> None:
    schema = ValidationSystemCandidateValidator(context=_context()).output_schema()
    ...
    assert "query-contract execution error, not an assertion failure" in (
        query_a_description
    )
...
async def test_candidate_builds_typed_group_and_static_value_check() -> None:
    validator = ValidationSystemCandidateValidator(context=_context())
...
async def test_candidate_reports_assertion_contract_failure_at_the_check() -> None:
    ...
    issues = (
        await ValidationSystemCandidateValidator(context=_context()).validate(
...
async def test_candidate_rejects_unsafe_or_non_row_comparison_sql() -> None:
    ...
    unsafe = await validator.validate(
        _candidate(query="DELETE FROM catalog.gold.dim_customer")
    )
    unqualified = await validator.validate(
        _candidate(query="SELECT count(*) FROM dim_customer")
    )
...
async def test_candidate_rejects_empty_group_list() -> None:
    validation = await ValidationSystemCandidateValidator(context=_context()).validate(
...
def test_reconciliation_uses_inactive_tombstones_for_omitted_applied_validation() -> (
    None
):
    legacy_group = _group().model_copy(
        update={"validation_group_name": "legacy_reconciliation"}
    )
    legacy_check = _check().model_copy(
        update={"validation_group_name": "legacy_reconciliation"}
    )
