...
@dataclass
class _StaticLogicalWorkflowService:
    changed: bool = True
...
def _client(service: _StaticLogicalWorkflowService) -> TestClient:
    app = FastAPI()
    app.include_router(
        create_logical_workflow_router(
...
def test_execute_endpoint_starts_without_process_local_execution() -> None:
    ...
    assert (
        ExecuteLogicalRunRequest.model_validate(
            {
                "execution_mode": "tool_assisted",
                "expected_model_revision": 4,
            },
...
def test_execute_endpoint_does_not_reschedule_an_already_started_one_shot_run() -> None:
    service = _StaticLogicalWorkflowService(changed=False)
...
@pytest.mark.asyncio
async def test_workflow_passes_the_internal_claim_only_to_logical_execution() -> None:
    lifecycle = _Lifecycle()
    executor = _Executor()
    service = LogicalWorkflow(lifecycle=lifecycle, executor=executor)
    principal = RequestPrincipal(
    ...
    assert lifecycle.expected_bindings == [("logical", "tool_assisted")]
