...
def enrichment_seeded_database(
...
class DescriptionAgent(LocalFakeAgentAdapter):
    ...
    async def execute(self, request: AgentExecutionRequest) -> AgentExecutionResult:
        ...
        groups = inputs["object_attribute_context"]
        assert isinstance(groups, list) and len(groups) == 1
        group = groups[0]
        assert isinstance(group, dict)
        assert group["selected_attribute_names"] == (
            [] if len(keys[0]) == 5 else [key[-1] for key in keys]
        )
        assert group["attributes"], "Sibling Attributes remain available as evidence."
        if self.behavior == "context_exhausted" and self.calls == 1:
            raise AgentExecutionFailedError("context_exhausted")
...
async def test_shared_executor_completes_physical_enrichment(
    enrichment_seeded_database: DisposablePostgres,
    behavior: str,
) -> None:
    database = enrichment_seeded_database
    ...
    try:
        ...
        claim = UUID(
            str(
                _claim_specific_workflow_run(database, run_id)[
    ...
    with database.connect_owner() as connection:
        ...
        if behavior == "context_exhausted":
            events = connection.execute(
                "SELECT model_event_log_stage AS stage,model_event_log_status AS status,"
                "model_event_log_message AS message,model_event_log_current AS current,"
                "model_event_log_total AS total,finding_count FROM model.model_event_log "
                "WHERE workflow_run_id=%s ORDER BY model_event_log_sequence",
                (run_id,),
            ).fetchall()
            units = [
                event for event in events if event["stage"] == "candidate_authoring"
            ]
            assert len(units) == agent.calls > 1
            failed = units[0]
            assert failed["status"] == "warning" and failed["current"] == 1
            assert failed["total"] == len(units) and failed["finding_count"] == 1
            safe_error = AgentExecutionFailedError("context_exhausted")
            assert failed["message"] == (
                f"Object description unit unavailable ({safe_error.code}): {safe_error.message}"
            )
            assert all(event["status"] == "running" for event in units[1:])
            assert units[1]["current"] == 2
            assert result.counts["unavailable"] == 1 and result.counts["applied"] > 0
            assert result.warning_count > 0
            assert any(
                event["stage"] == "metadata_enrichment" and event["status"] == "warning"
                for event in events
            )
        if behavior == "regenerate_bulk_attributes":
            after_objects = {
