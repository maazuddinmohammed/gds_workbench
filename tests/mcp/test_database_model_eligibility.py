...
def test_model_object_eligibility_routes_active_scoped_zones(
        ...
        connection.execute(
            ...
            SELECT %s, object.object_id
             ...
             WHERE object.object_schema IN (
                       'bronze_demo', 'silver_demo', 'gold_demo'
                   )
            """,
            (model_id,),
        )
    ...
    assert _required_bool(by_zone["silver"], "is_bronze_source_eligible") is False
    ...
    assert _required_bool(by_zone["gold"], "is_bronze_source_eligible") is False
...
def test_model_attribute_eligibility_routes_active_scoped_zones(
    ...
    assert set(by_zone) == {"bronze", "silver", "gold"}
    assert all(
        _required_bool(row, "is_bronze_source_eligible") for row in by_zone["bronze"]
    )
    assert all(
    ...
    with postgres_database.connect_owner() as connection:
        ...
        connection.execute(
            ...
            UPDATE core.tenant_metadata_discovery_scope
               SET is_active = FALSE
             WHERE tenant_id = %s
               AND lower(btrim(object_schema)) = 'bronze_demo'
            """,
            (tenant_id,),
        )
...
def test_gds_eligibility_uses_assigned_tenant_when_connection_owner_is_inactive(
    ...
    assert len(rows) == 1
    assert _required_int(rows[0], "object_tenant_id") == tenant_id
    assert _required_bool(rows[0], "is_bronze_source_eligible") is True
...
def test_only_active_logical_mapping_enables_silver_dimensional_eligibility(
    ...
    with postgres_database.connect_owner() as connection:
            ...
            _required_row(
                connection.execute(
                    ...
                    INSERT INTO workflow.logical_entity (
                        ...
                        %s, 'customer', 'Customer', 'core', 'One customer', 'needs_review'
                    )
            ...
            _required_row(
                connection.execute(
                    ...
                    INSERT INTO workflow.logical_attribute (
                        ...
                        %s, %s, 'customer_id', 'Customer ID', 'bigint', 1, 'needs_review'
                    )
        ...
        connection.execute(
            ...
            ) VALUES (%s, 'logical_entity', %s, 'needs_review')
            """,
        ...
        mapping_object_id = _required_int(
            _required_row(
                connection.execute(
                    ...
                    ) VALUES (%s, %s, %s, 'logical_entity', %s, 'needs_review')
                    RETURNING mapping_object_id
                    """,
                    (model_id, silver_object_id, source_system_id, logical_entity_id),
                ).fetchone()
            ),
            "mapping_object_id",
        )

        object_row = _required_row(
            connection.execute(
                "SELECT * FROM workflow.list_model_object_eligibility(%s)",
                (model_id,),
            ).fetchone()
        ...
        connection.execute(
            ...
            ) VALUES (%s, %s, %s, %s, 'logical_entity', %s, 'needs_review')
            """,
            (
                model_id,
                silver_object_id,
        ...
        attribute_needs_review = _required_row(
            connection.execute(
                """
                SELECT *
        ...
        connection.execute(
            ...
            UPDATE workflow.logical_entity
               SET logical_entity_status = 'active'
        ...
        connection.execute(
            ...
            UPDATE workflow.logical_attribute
               SET logical_attribute_status = 'active'
             WHERE logical_attribute_id = %s
            """,
            (logical_attribute_id,),
        )
        connection.execute(
            ...
            UPDATE workflow.mapping_source_system_dependency
               SET mapping_source_system_dependency_status = 'active'
             WHERE model_id = %s
        ...
        connection.execute(
            ...
            UPDATE workflow.mapping_attribute
             ...
             WHERE mapping_object_id = %s
               AND attribute_id = %s
            """,
            (mapping_object_id, silver_attribute_id),
        )
    ...
    assert _required_bool(object_row, "is_dimensional_source_eligible") is False
    assert _required_bool(attribute_before, "is_dimensional_source_eligible") is False
    assert (
        _required_bool(attribute_needs_review, "is_dimensional_source_eligible")
        is False
    )
