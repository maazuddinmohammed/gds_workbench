...
Inventory totals: **94 tables, 88 functions, and 16 installed triggers**.
...
### `application` — Workflow Run control (23)
...
- `application.start_workflow_run` — Start a caller-owned queued Run. Steps: (1) lock Run and active Model; (2) authorize and verify actor ownership; (3) replay non-queued state without mutation; (4) verify Model revision; (5) reject when another Run is active for the Tenant, with a unique-index race fence; (6) move to `running`; (7) append sequence-1 `started` event and return.
...
- `application.assert_notebook_workflow_run_claim` — Actor-bound notebook write fence. Steps: (1) resolve the bound workload Principal; (2) lock and require its exact Run; (3) delegate token digest and expiry verification to the governed claim assertion.
- `application.resolve_notebook_profiling_claim` — Private notebook Profiling execution fence. Steps: (1) resolve the actor from the bound database login; (2) lock the exact active Tenant/Model/Profiling Run at its frozen base revision; (3) require the exact actor Principal and Entra identity; (4) verify the live unexpired claim token digest; (5) return only the resolved actor/environment record to trusted wrappers.
...
- `application.fail_workflow_run` — Fail a running Run safely. Steps: (1) validate bounded code/message; (2) lock/authorize Run and verify owner/current revision; (3) reject Runs with durable no-op or validated/applied authoring outcome; (4) replay only an exact previous failure; (5) require running state; (6) set failed state, clear claim, append failure event, and return.

### `application` — execution context and governed result storage (12)
...
- `application.get_notebook_profiling_execution_context` — Claimed notebook Profiling context wrapper. Steps: (1) bind Tenant, Model, Run, revision, actor identity, and live claim through the private fence; (2) call the governed Profiling context with server-derived identity; (3) preserve Tenant Lock and physical-scope validation.
- `application.get_notebook_profiling_connection_values` — Claimed notebook Profiling connection wrapper. Steps: (1) apply the exact claim fence; (2) derive Environment from the database-owned notebook binding; (3) call the all-or-nothing governed connection lookup without a caller-selectable identity or Environment.
- `application.append_notebook_profiling_event` — Claimed notebook progress wrapper. Steps: (1) apply the exact claim fence; (2) fix Profiling attempt to one; (3) delegate safe message, contiguous sequence, progress, revision, authorization, and idempotency checks to the governed event append.
- `application.persist_and_complete_notebook_profiling_run` — Atomic notebook Profiling commit. Steps: (1) apply the exact claim fence; (2) persist the existing exact Profile result shape and its recomputed source-context digests; (3) complete the same Run using the resulting Model revision/count; (4) roll back both operations on any error.
- `application.fail_notebook_profiling_run` — Claimed notebook Profiling failure wrapper. Steps: (1) apply the exact claim fence; (2) delegate bounded failure code/message, Tenant Lock, revision, durable-outcome, and terminal event rules to the governed Run failure function.
...
- `mcp.create_metadata_change_set` — Create one ongoing Tenant draft per Principal. Steps: (1) validate required identity/ID inputs; (2) lock active Tenant and authorize metadata write/owned lock; (3) lock any creator-owned active/validated draft and expire it plus active batches with one event when due; (4) return a live draft with `metadata_change_set_exists`; (5) otherwise insert an empty sixteen-dataset draft and append `created`; (6) return state and expiry.
- `mcp.stage_metadata_change_set` — Replace one or more complete Metadata datasets. Steps: (1) validate required inputs and a bounded object containing only the 16 allowed arrays; (2) authorize metadata write and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse staging; (4) require active/validated state and exact revision; (5) replace supplied arrays, reset validation, increment revision, and refresh expiry; (6) append dataset counts and return.
- `mcp.begin_metadata_stage_batch` — Start/replay a chunk manifest for one dataset. Steps: (1) validate all required manifest inputs; (2) authorize and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse begin; (4) expire stale active batches for the dataset; (5) replay an identical active manifest or return conflict; (6) insert a batch bounded by draft expiry and return progress.
- `mcp.put_metadata_stage_chunk` — Add one immutable chunk. Steps: (1) validate required index, digest, dataset, and bounded nonempty array; (2) authorize and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse put; (4) lock and validate the exact active batch, expiring it when due; (5) replay the exact chunk or reject conflict/overflow; (6) insert the chunk, touch the live batch, and return progress.
- `mcp.commit_metadata_stage_batch` — Assemble and commit a complete batch. Steps: (1) validate required IDs/revision; (2) authorize and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse commit; (4) lock/replay the exact committed batch or require a live exact-revision batch; (5) verify manifest counts/hash and assemble bounded ordered records; (6) call the same atomic Stage operation; (7) mark the batch committed with resulting revision/expiry and return.
- `mcp.get_metadata_change_set` — Creator-scoped lifecycle read. Steps: (1) validate required identity/ID inputs; (2) authorize Tenant lock management without requiring lock ownership; (3) lock the exact creator/Tenant draft; (4) atomically expire a due draft/batches with one event using the stored correlation ID; (5) return all 16 documents and lifecycle fields or a bounded denial/not-found row.
- `mcp.record_metadata_change_set_validation` — Seal or unseal validation. Steps: (1) validate required revision, boolean, outcome object, digest shape, and correlation; (2) authorize metadata write and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse validation; (4) require active/validated state and exact revision; (5) seal candidate digest on success or return to active on failure, then refresh expiry; (6) append the validation event and return.
- `mcp.archive_metadata_change_set` — Retained terminal archive. Steps: (1) validate required identity/ID/revision/correlation inputs; (2) authorize lock management and lock the creator-owned draft; (3) atomically expire a due draft/batches with one event and refuse archive; (4) require active/validated state and exact revision; (5) set retained `archived` terminal state and append its event; (6) return terminal state.
...
- `guard_workflow_run` on `application.workflow_run` — UPDATE/DELETE, row. Steps: (1) reject DELETE and frozen Tenant/request mutation; (2) reject terminal mutation; (3) validate claim recovery/rotation while running; (4) enforce queued→running→terminal state flow.
