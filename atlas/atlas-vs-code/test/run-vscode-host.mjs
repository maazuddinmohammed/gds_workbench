...
const vsix = path.resolve(process.argv[2] ?? path.join(root, '../dist', `atlas-stage-runner-${version}.vsix`));
...
writeFileSync(path.join(user, 'User/settings.json'), JSON.stringify({
  ...
  'atlas.stageRunner.profile': 'local',
...
const code = process.env.ATLAS_STAGE_TEST_CODE ?? (process.platform === 'darwin'
