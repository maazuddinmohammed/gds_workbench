...
const {fields, norm, key, ownKey, attributeKey, maskingKeys, quote, normalizedType, stringType, scalarType, planProfiling, batchPredicate} = require("./profiling.js");
...
function planAnalysis(metadata, scope, plan) {
  exactFields(plan, ["batches", "probes"], "Analysis plan");
  if (!Array.isArray(plan.probes) || plan.probes.length < 1 || plan.probes.length > 50) throw Error("Select 1–50 Analysis probes.");
  const requested = new Map();
  for (const probe of plan.probes) for (const endpoint of probe.kind === "join" ? [probe.from, probe.to] : [{object: probe.object}]) {
    if (endpoint?.object) requested.set(key(endpoint.object), endpoint.object);
  }
  const resolved = planProfiling(metadata, scope, {...plan.batches,
    selected_objects: plan.batches?.selected_objects ?? [...requested.values()]});
  const batches = new Map(resolved.queries.map((query) => [key(query.object), query.batch]));
  ...
  return plan.probes.map((probe) => {
    ...
    for (const endpoint of endpoints) {
      if (!batches.has(key(endpoint.object))) throw Error("Analysis endpoint has no resolved profiling/batch scope.");
      endpoint.batch = batches.get(key(endpoint.object));
    }
    ...
    return {id: probe.id, kind, scope: endpoints.some((endpoint) => endpoint.batch) ? "selected_batches" : "all_rows", execution_state: "not_run", scan_cost: "unknown", max_result_rows: 1,
