...
function Resolve-AnalysisEndpoint($Metadata, $Scope, $Endpoint, $Masked) {
    ...
    $objects = @(@((Get-Property $Metadata 'source_object')) + @((Get-Property $Metadata 'bronze_object')) | Where-Object { $null -ne $_ -and ((Get-Active $_) -ne $false) })
    ...
    $connections = @((Get-Property $Metadata 'connection') | Where-Object { ((Get-Active $_) -ne $false) -and
    ...
    $tenants = @((Get-Property $Metadata 'tenant') | Where-Object { ((Get-Active $_) -ne $false) -and
...
function Build-AnalysisSql([string]$Kind, $Endpoints, [int]$DeterminantCount) {
    ...
    $scoped = {
        ...
        $ctes.Add("$Name AS (`nSELECT " + ($projected -join ', ') + "`n  FROM " + $Endpoint.relation + (Get-ProfileBatchPredicate (Get-Property $Endpoint 'batch')) + "`n)")
        return ,$columns
    }
...
function Plan-Analysis($Metadata, $Scope, $Plan) {
    Assert-AnalysisFields $Plan @('batches','probes') 'Analysis plan' 
    ...
    foreach ($probe in $Plan.probes) {
        $selected = if ((Get-Property $probe 'kind') -ceq 'join') { @((Get-Property $probe 'from'), (Get-Property $probe 'to')) } else { @($probe) }
        foreach ($item in $selected) {
            $object = Get-Property $item 'object'
            if ($null -ne $object) { $requested[(Get-ProfileKey $object)] = $object }
        }
    }
    $batchPlan = @{}
    foreach ($name in @(Get-PropertyNames $Plan.batches)) { $batchPlan[$name] = Get-Property $Plan.batches $name }
    if (-not $batchPlan.ContainsKey('selected_objects')) { $batchPlan['selected_objects'] = @($requested.Values) }
    $resolved = Plan-Profiling $Metadata $Scope $batchPlan
    $batches = @{}
    foreach ($query in $resolved.queries) { $batches[(Get-ProfileKey $query.object)] = $query.batch }
    ...
    foreach ($probe in $Plan.probes) {
        ...
        foreach ($endpoint in $endpoints) {
            $identity = Get-ProfileKey $endpoint.object
            if (-not $batches.ContainsKey($identity)) { Fail 'Analysis endpoint has no resolved profiling/batch scope.' }
            $endpoint['batch'] = $batches[$identity]
        }
        ...
        $publicEndpoints = @(foreach ($endpoint in $endpoints) {
            [ordered]@{object=$endpoint.object;source_tenant_code=$endpoint.source_tenant_code;source_system_codes=$endpoint.source_system_codes;columns=$endpoint.columns;batch=$endpoint.batch}
        })
        $scanScope = if (@($endpoints | Where-Object { $null -ne $_.batch }).Count) { 'selected_batches' } else { 'all_rows' }
        $query = [ordered]@{id=$probe.id;kind=$kind;scope=$scanScope;execution_state='not_run';scan_cost='unknown';max_result_rows=1;endpoints=$publicEndpoints}
