    ...
    for (const [datasetName, value] of loaded) {
      ...
      for (const record of value.baseline || []) {
        try {
          ...
          if (baseline.has(key)) issues.push({ code: "duplicate_snapshot_key", dataset: datasetName,
            message: "Snapshot records contain a duplicate canonical key; obtain a corrected Snapshot." });
          baseline.set(key, record);
        } catch (error) {
          issues.push({ code: "snapshot_canonical_key", dataset: datasetName,
            message: `Snapshot identity is malformed: ${error.message}` });
        }
      }
