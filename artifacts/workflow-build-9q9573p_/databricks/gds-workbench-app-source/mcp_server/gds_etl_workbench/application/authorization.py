...
class AuthorizationService:
    ...
    async def authorize_tenant(
        ...
        *,
        ...
        """Authorize Tenant policy and, for Model operations, retained source access."""
        authorization = await self._authorize_tenant(
            transaction, request_principal, tenant_id=tenant_id, policy=policy
        )
        if model_id is None:
        ...
        return replace(authorization, readable_source_tenant_ids=readable)
...
async def readable_tenant_ids(
    transaction: ReadTransaction, principal: ResolvedPrincipal
) -> tuple[int, ...]:
    """Reuse Tenant visibility, including membership expiry and development policy."""
    from gds_etl_workbench.application.tenants import query_visible_tenants

    result: list[int] = []
    while True:
        rows = await query_visible_tenants(transaction, principal, limit=1000, offset=len(result))
        result.extend(row["tenant_id"] for row in rows)
        if len(rows) < 1000:
            return tuple(result)
