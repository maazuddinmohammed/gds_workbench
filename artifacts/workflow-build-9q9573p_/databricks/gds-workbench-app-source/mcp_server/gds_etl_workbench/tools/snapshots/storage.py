"""Shared private Azure Blob persistence for governed Snapshot archives."""
            ...
            with archive.path.open("rb") as archive_file:
                await blob.upload_blob(
                    ...
                    content_settings=ContentSettings(
                        ...
                        content_disposition=(
                            f'attachment; filename="{snapshot_kind}-snapshot-'
                            f'{scope_id}-{snapshot_id}.zip"'
                        ),
                    ),
                )
    ...
    async def create_read_url(
        ...
        *,
        ...
        try:
            ...
            expires = min(current + timedelta(seconds=ttl_seconds), available_until)
...
def _scope_field(snapshot_kind: SnapshotKind) -> str:
    return "tenant_id" if snapshot_kind == "metadata" else "model_id"
