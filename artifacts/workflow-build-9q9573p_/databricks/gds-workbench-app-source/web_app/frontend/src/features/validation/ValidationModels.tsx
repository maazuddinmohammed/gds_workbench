...
export function ValidationModels({ api, tenantId }: { api: ValidationApi; tenantId: number }) {
  const query = useInfiniteQuery({
    queryKey: validationQueryKeys.models(tenantId),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link
          ...
          aria-label={`Open ${row.original.model_name} Validation`}
  ...
  return (
    <section className="models-page validation-models page-enter">
      <header className="models-commandbar">
        <div>
          <p className="eyebrow">Applied validation register</p>
          <h1>Validation</h1>
        </div>
        <div className="mapping-model-actions">
          <span className="models-context-note">Choose an active Model to review or author Validation</span>
          <button
            className="button button-secondary button-small"
            type="button"
      ...
      {query.isPending ? (
        <div className="surface-state" aria-busy="true">Loading Models for Validation…</div>
      ) : query.error instanceof ApiError && query.error.status === 403 ? (
        <div className="surface-state is-error" role="alert">
          You do not have permission to view Models for Validation.
        </div>
      ) : query.isError ? (
        <div className="surface-state is-error" role="alert">
          Models for Validation could not be loaded.
        </div>
      ) : models.length === 0 ? (
        <div className="empty-state compact">No active Models are available for Validation.</div>
      ) : (
        <div className="models-table-scroll table-scroll">
          <table aria-label="Models for Validation">
            <thead>
